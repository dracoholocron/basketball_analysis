...
async def test_list_training_sessions(client, auth_headers, training_session):
    r = await client.get("/api/v1/training", headers=auth_headers)
...
async def test_create_training_session(client, auth_headers):
    r = await client.post("/api/v1/training", json={"sport_drill": "Jump Shot"}, headers=auth_headers)
...
async def test_get_training_session(client, auth_headers, training_session):
    r = await client.get(f"/api/v1/training/{training_session.id}", headers=auth_headers)
    assert r.status_code == 200
    assert r.json()["id"] == str(training_session.id)


async def test_delete_training_session(client, auth_headers, training_session):
    r = await client.delete(f"/api/v1/training/{training_session.id}", headers=auth_headers)
...
async def test_analyze_session_enqueues_task(client, auth_headers, db_session, admin_user):
    ...
    with patch("app.worker.gpu_tasks.run_pose_analysis_task", mock_task, create=True):
        r = await client.post(f"/api/v1/training/{s.id}/analyze", headers=auth_headers)
...
async def test_get_keypoints(client, auth_headers, db_session, training_session):
    kp = PoseKeypoints(
        ...
        keypoints=[[100, 200]], bbox={"x1": 10, "y1": 20, "x2": 100, "y2": 200},
    )
    db_session.add(kp)
    await db_session.commit()
